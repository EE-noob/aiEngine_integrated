...
__STATIC_FORCEINLINE void riscv_memcpy_s8_asm_unroll4(
    void *__RESTRICT dst,
    ...
    uint32_t bytes)
{
    ...
    /* 对齐到 4 字节边界，避免非对齐 lw/sw */
    while (((uintptr_t)d & 3) && bytes) {
        *d++ = *s++;
        bytes--;
    }

    uint32_t words = bytes >> 2;   /* 32-bit 字数 */
    uint32_t rem   = bytes & 3;    /* 剩余字节数 */
