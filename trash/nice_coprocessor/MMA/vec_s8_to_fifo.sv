...
module vec_s8_to_fifo #(
    ...
    always @(*) begin
        if (!rst_n) begin
            ...
            if (fifo_wptr0 >= output_ptr0)
                fifo_count0 = fifo_wptr0 - output_ptr0;
            else
                fifo_count0 = VLEN - output_ptr0 + fifo_wptr0;
            if (fifo_wptr1 >= output_ptr1)
                fifo_count1 = fifo_wptr1 - output_ptr1;
            else
                fifo_count1 = VLEN - output_ptr1 + fifo_wptr1;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) cnt_each_row <= 'b0;
            ...
            if (cnt_each_row == (vec_valid_num_col_tmp/4))
                cnt_each_row <= 'b0;
            else
                cnt_each_row <= cnt_each_row + 1'b1;
        end
    end
    ...
    always @(*) begin
        ...
        if (stream_active) begin
            if (fifo_read_sel == 1'b0) begin
                output_data = fifo_mem0[output_ptr0][(3-cnt_each_row)*32 +: 32];
                output_mask = stored_valid_col[(3-cnt_each_row)*4 +: 4];
            end else begin 
                output_data = fifo_mem1[output_ptr1][(3-cnt_each_row)*32 +: 32];
                output_mask = stored_valid_col[(3-cnt_each_row)*4 +: 4];
            end
        end
    end
