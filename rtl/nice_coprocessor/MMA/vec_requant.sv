...
module vec_requant #(
  ...
  // 状态机与游标
  // ----------------------------
  typedef enum logic [1:0] {IDLE, LOAD, COMPUTE, TILE_COMPLETE} state_e;
  state_e state, state_n;

  // 只保留 4 个状态名，LOAD 内部用 phase 区分 mul/shift
  typedef enum logic [0:0] {PH_MUL=1'b0, PH_SHIFT=1'b1} phase_e;
  ...
  // tile 列需要多少 lane（尾块）
  // ----------------------------
  function automatic [31:0] f_lane_need(
    input [31:0] cols, input [31:0] vlen, input [31:0] tcol
  );
    reg [31:0] remain;
    begin
  ...
  function automatic [31:0] f_rows_need(
    input [31:0] rows, input [31:0] vlen, input [31:0] trow
  );
    reg [31:0] remain;
    begin
  ...
  always_ff @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      // per-channel 模式下：在 quant_params_valid 时锁存当前需要的 lanes
      ...
      if (init_cfg) begin
        ...
        LOAD: begin
          ...
          if (icb_rsp_s.rsp_valid && icb_rsp_m.rsp_ready && cmd_busy) begin
            ...
            if (load_phase==PH_MUL)
              ch_multiplier_r[rd_beats_cnt] <= icb_rsp_s.rsp_rdata;
            else
              ch_shift_r[rd_beats_cnt]      <= icb_rsp_s.rsp_rdata;
  ...
  always_ff @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      in_valid_q <= in_valid && ( (!cfg_per_channel) || quant_params_valid );
      out_valid_q  <= in_valid_q;
    end
  end
  ...
  for (j=0; j<VLEN; j++) begin : LANE
    ...
    logic signed [31:0] a_shifted, high_mul, rq;
    logic        [5:0]  right_s;
    ...
    always_ff @(posedge clk or negedge rstn) begin
      if (!rstn) begin
        ...
        if (lane_en) begin
          ...
          else if (rq < -32'sd128) out_vec_s8[j] <= -32'sd128;
